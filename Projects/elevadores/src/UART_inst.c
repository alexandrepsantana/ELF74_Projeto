...
//
typedef enum {
  S0000,        // Reset dos Elevadores
  S0005,        // Elevador Livre; Aguardo de Chamada
  S0010,        // Verificação se o Botão é no mesmo Andar que os Elevadores
  S0020,        // Abertura da Porta Interna dos Elevadores
  S0030,        // Escolha do Elevador de Preferência
  S0040,        // Prepara movimento do Elevador (decide a Direção,
  ...
  S0085,        // Atualiza Alvo
  S0090,        // Limpeza do Botão de Chamada
  S0095         // Atualiza Alvo        
} state_t;

typedef struct stElevatorMsg {
  osMessageQueueId_t     q;
  uint8_t                number;                     // Número do Elevador
  int8_t                 direction;                  // Direção
  uint8_t                target;                     // Alvo
  uint8_t                currentStage;               // Andar atual do Elevador
  uint8_t                doorState;                  // Estado da Porta Interna do Elevador
  uint8_t                *extButtons;                // Botões Externos chamados
  uint8_t                *intButtons;                // Botões Internos chamados
  struct stElevatorMsg** elevators;                  // Os três elevadores.
} elevatorMsg_t;
...
// Eventos provenientes do Estado Físico.
...
// Comandos de atuação sobre o Estado Físico.
...
ControlTask(void *arg)
{
  ...
  // Declarar e inicializar Botôes Externos.
  ...
  while (FOREVER) {
    ...
    if ((elevatorNumber = ElevatorNumberEventHelper(buffer[0])) != 0xFF) {
      // Conseguiu o Número do Elevador.
      // Inicializar arg.
      ...
      if (eventArg.stage != 0xFF && eventArg.elevatorNumber != 0xFF) {
        ...
        case OUTERCALL:
          ...
          break;
        default:
          break; // Ignora caracter estranho. Não faz nada.
        }
      }
    }
  }
...
ElevatorTask(void* arg)
{
  ...
  while (FOREVER) {
    if (osOK == osMessageQueueGet(msg.q, (elevatorMsg_t*)pMsg, NULL, 0)) {
      // Se veio mensagem, atualizar tanto as variáveis locais quanto
      // a mensagem original.
    ...
    case S0000:
      {
        ...
        if (AbertaPortaElevador(msg.currentStage, pMsg)) {
          ...
          // Se não houve Chamada no mesmo Andar que o Andar atual do
          // Elevador, então transitar para o Estado de escolha do Elevador
          // de Preferência.
          state = S0030;
        }
        break;
    ...
    case S0030:
//      {
//        // Verificar qual dos Elevadores é o de Preferência.
//        uint8_t number = VerificarPreferencia(msg.elevators);
//
//        if (number == msg.number) {
//          // O Elevador de Preferência é o atual.
//          state = S0040;
//        }
//        break;
//      }
...
AbertaPortaElevador(uint8_t ui8Stage, elevatorMsg_t* elevator) 
{
  ...
  if (ui8Stage == elevator->currentStage &&
    ...
    // Elevador está no Andar em que ocorreu Chamada.
    // Abrir a Porta Interna, como emergência.
    commandArg_t arg;
...
{
  // O Alvo é o Andar com Chamada o mais distante possível
  // do Andar atual do Elevador.
  uint8_t result = 0xFF;
  uint8_t absDistance = 0; // Valor mínimo possível.
  ...
  for (uint8_t i = NUMBEROFSTAGES; i > 0; --i) {
    if (elevator->intButtons[i - 1] || elevator->extButtons[i - 1]) {
      // O botão foi pressionado no Andar i.
      if (abs(i - 1 - currentStage) > absDistance && 
          SemAlvosIguaisA(i - 1, elevator->elevators, elevator->number)) {
        // A distância até a Chamada no Andar i é a maior de todas.
        result = i - 1;
  ...
  if (result != 0xFF) {
    // Foi detectado um possível Alvo.
...
uint8_t
SemAlvosIguaisA(uint8_t ui8Target, elevatorMsg_t** elevators, uint8_t ui8ElevatorNumber)
{
  uint8_t result = TRUE;
...
uint8_t
AtendeChamada(elevatorMsg_t* elevator)
{
  uint8_t result = FALSE;
  uint8_t currentStage = elevator->currentStage;
  ...
  if (elevator->intButtons[currentStage] ||
      ...
      // Atender à Chamada no Andar do Elevador.
      result = TRUE;
...
//------
//
// Rotina de tratamento de interrupção da UART.
//
void
UART0_Handler(void)
{
...
ChamadaInterna(eventArg_t arg)
{
  ...
  // Ligar Botão Interno caso o Andar do Botão Interno não seja o Andar
  // atual do Elevador.
...
//
uint8_t
ElevatorNumberEventHelper(uint8_t ui8Char)
{
  uint8_t value;
  return (value = ui8Char - 'c') >= NUMBEROFELEVATORS 
    ? 0xFF 
    : value;
}

uint8_t 
StageCharEventHelper(uint8_t ui8Char)
{
  uint8_t value;
  return (value = ui8Char - 'a') >= NUMBEROFSTAGES 
    ? 0xFF 
    : value;
}

uint8_t 
NumericStageEventHelper(const char *sentence)
{
  return (*sentence >= '0' && *sentence <= '9')
    ? atoi(sentence)
    : 0xFF;
}

char 
ElevatorCharCommandHelper(uint8_t ui8ElevatorNumber)
{
  return ui8ElevatorNumber >= NUMBEROFELEVATORS
    ? 0xFF
    : ui8ElevatorNumber + 'c';
}

char 
StageCharCommandHelper(uint8_t ui8StageNumber)
{
  return ui8StageNumber >= NUMBEROFSTAGES
    ? 0xFF
    : ui8StageNumber + 'a';
}
...
void
DesligarBotao(commandArg_t arg)
{
  osMutexAcquire(g_uartMutex, osWaitForever);
  UARTprintf("%c", ElevatorCharCommandHelper(arg.elevatorNumber));
